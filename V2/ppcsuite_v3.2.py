...
# Firebase integration (optional - graceful fallback if not configured)
try:
    import firebase_config
    import importlib
    # CRITICAL: Force reload to pick up code changes without restarting server
...
if 'firebase_initialized' not in st.session_state:
    ...
    if FIREBASE_MODULE_AVAILABLE:
        try:
            if FirebaseDB.initialize(st.secrets):
                st.session_state['firebase_initialized'] = True
            else:
                st.session_state['firebase_error'] = FirebaseDB.get_last_error()
...
# ==========================================
# DATE RANGE DETECTION & WEEKLY NORMALIZATION
# ==========================================
...
def detect_date_range(df: pd.DataFrame, col_map: dict = None) -> dict:
    ...
    result = {
        ...
        'end': None,
        'days': 0,
        'weeks': 1.0,  # Default to 1 week (no division) if can't detect
    ...
    # Method 1: Use SmartMapper result if available
    if col_map and col_map.get('Date'):
        date_candidates.append(col_map['Date'])
    
    # Method 2: Look for common date column names
    date_aliases = ['start date', 'end date', 'date', 'day', 'report date']
    for col in df.columns:
        if any(alias in str(col).lower() for alias in date_aliases):
    ...
    for date_col in date_candidates:
        ...
        try:
            ...
            if len(dates) > 0:
                ...
                if min_date.year >= 2020 and max_date.year <= 2030:
                    days = (max_date - min_date).days + 1
                    weeks = max(days / 7, 1.0)  # Minimum 1 week
                    ...
                    result = {
                        ...
                        'days': days,
                        'weeks': weeks,
                        'label': f"{min_date.strftime('%b %d')} - {max_date.strftime('%b %d, %Y')} ({weeks:.1f} weeks)"
                    }
                    break  # Found valid dates, stop looking
        except Exception:
            continue
...
def normalize_to_weekly(metrics: dict, weeks: float) -> dict:
    ...
    # Volume metrics that should be divided
    volume_keys = ['spend', 'sales', 'clicks', 'orders', 'impressions']
    
    normalized = metrics.copy()
    for key in volume_keys:
        if key in normalized and isinstance(normalized[key], (int, float)):
            normalized[key] = normalized[key] / weeks
    
    # Recalculate ratio metrics from normalized volumes
    if normalized.get('spend', 0) > 0:
        normalized['roas'] = normalized.get('sales', 0) / normalized['spend']
    if normalized.get('sales', 0) > 0:
        normalized['acos'] = (normalized.get('spend', 0) / normalized['sales']) * 100
    if normalized.get('clicks', 0) > 0:
        normalized['cpc'] = normalized.get('spend', 0) / normalized['clicks']
        normalized['cvr'] = normalized.get('orders', 0) / normalized['clicks']
    if normalized.get('impressions', 0) > 0:
        normalized['ctr'] = (normalized.get('clicks', 0) / normalized['impressions']) * 100
...
def simulate_bid_changes(
    ...
    Args:
        ...
        bids_dict: Dict containing bid recommendations
        config: Configuration dict
        date_info: Date range info for weekly normalization (optional)
    """
    
    # Default date_info if not provided
    if date_info is None:
        date_info = {'weeks': 1.0, 'label': 'Period Unknown'}
    ...
    # Calculate current baseline (raw totals)
    current_summary_raw = calculate_current_baseline(df)
    
    # Apply weekly normalization to current baseline
    current_summary = normalize_to_weekly(current_summary_raw, num_weeks)
    ...
    for scenario_name, elasticity in elasticity_scenarios.items():
        forecast_raw = forecast_scenario_fixed(
            combined_bids,
            harvest_df,
            elasticity, 
            current_summary_raw,  # Pass raw for delta calculations
            config
        )
        # Normalize the forecast to weekly
        forecast = normalize_to_weekly(forecast_raw, num_weeks)
        # Preserve diagnostic keys (start with _)
        for key, val in forecast_raw.items():
            if key.startswith('_'):
                forecast[key] = val
        scenarios[scenario_name] = forecast
    
    # Add current baseline (already normalized)
    scenarios['current'] = current_summary
    
    # Add diagnostics and date info
    for scenario in scenarios.values():
        ...
        scenario['_date_info'] = date_info
    
    # Calculate sensitivity (normalized)
    sensitivity_raw = calculate_sensitivity_fixed(
        combined_bids, harvest_df, 
        elasticity_scenarios['expected'], 
        current_summary_raw, config
    )
    # Normalize sensitivity results if it's a DataFrame
    if isinstance(sensitivity_raw, pd.DataFrame) and not sensitivity_raw.empty:
        for col in ['Spend', 'Sales', 'Orders']:
            if col in sensitivity_raw.columns:
                sensitivity_raw[col] = sensitivity_raw[col] / num_weeks
    ...
    return {
        ...
        'date_info': date_info  # Include for UI access
    }
...
def run_optimizer_logic(file_content, config):
    ...
    # NEW: Detect date range for weekly normalization
    col_map = SmartMapper.map_columns(df)
    date_info = detect_date_range(df, col_map)
    ...
    # NEW: Bid simulation with date info for weekly normalization
    simulation_results = simulate_bid_changes(df, bids_dict, config, date_info)
        ...
        if err:
            ...
            with t8:
                ...
                if simulation and simulation.get('scenarios'):
                    ...
                    # Get date info for context
                    date_info = simulation.get('date_info', {})
                    date_label = date_info.get('label', 'Period Unknown')
                    num_weeks = date_info.get('weeks', 1.0)
                    
                    # Show date range context
                    st.info(f"📅 **Data Period:** {date_label} — All metrics normalized to weekly averages")
                    
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        st.markdown("**Current Performance (Weekly Avg)**")
                        m1, m2 = st.columns(2)
                        m1.metric("Avg Weekly Spend", f"${current.get('spend', 0):,.0f}")
                        m2.metric("Avg Weekly Sales", f"${current.get('sales', 0):,.0f}")
                        m3, m4 = st.columns(2)
                        m3.metric("ROAS", f"{current.get('roas', 0):.2f}x")
                        m4.metric("Avg Weekly Orders", f"{current.get('orders', 0):.0f}")
                    
                    with col2:
                        st.markdown("**Forecasted Performance (Weekly Avg)**")
                        ...
                        m1, m2 = st.columns(2)
                        m1.metric("Avg Weekly Spend", f"${expected.get('spend', 0):,.0f}", f"{spend_change:+.1f}%")
                        m2.metric("Avg Weekly Sales", f"${expected.get('sales', 0):,.0f}", f"{sales_change:+.1f}%")
                        m3, m4 = st.columns(2)
                        m3.metric("ROAS", f"{expected.get('roas', 0):.2f}x", f"{roas_change:+.1f}%")
                        m4.metric("Avg Weekly Orders", f"{expected.get('orders', 0):.0f}", f"{orders_change:+.1f}%")
                        
                        st.caption(f"Confidence: 70% probability | Based on {num_weeks:.1f} weeks of data")
